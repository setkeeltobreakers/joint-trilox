...
int runPrompt() {
  ...
  while (1) {
    ...
    printf("%s", line);
    hadError = interpret(line, "REPL", &vim);
    if (hadError) initVM(&vim);
    free(line); 
  }
  freeVM(&vim);
}
